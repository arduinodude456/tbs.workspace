...
import { archiveApp, createApp, getActiveApps, getAllApps, getMailboxMessages, getUserById, getUsersForAdmin, markMailRead, sendInternalMail, tbsMailAddress, toggleMailStar } from "./db";
...
export const appRouter = router({
  ...
  mail: router({
    ...
    send: protectedProcedure.input(z.object({ recipientAddress: z.string().trim().regex(/^u[1-9][0-9]*@tbs\.workspace$/, "Bitte eine interne TBS-Adresse wie u2@tbs.workspace verwenden."), subject: z.string().trim().min(1).max(240), body: z.string().trim().min(1).max(10000) })).mutation(async ({ ctx, input }) => {
      const recipientId = Number(input.recipientAddress.match(/^u([1-9][0-9]*)@tbs\.workspace$/)?.[1]);
      const recipient = await getUserById(recipientId);
      if (!recipient || tbsMailAddress(recipient.id) !== input.recipientAddress) throw new TRPCError({ code: "NOT_FOUND", message: "Diese Adresse gehört keinem registrierten TBS-Nutzer. Externe Empfänger sind nicht möglich." });
      await sendInternalMail(ctx.user, recipient, input);
